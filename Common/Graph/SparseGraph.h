...
public:
  ...
  //non const class used to iterate through the nodes in the graph
    ...
    private:
      ...
      bool GetNextValidNode(typename NodeVector::iterator& it)
      {
        if ( curNode == G.m_Nodes.end() || it->Index() != invalid_node_index) return false;

        while ( (it->Index() == invalid_node_index) )
        {
          ++it;

          if (curNode == G.m_Nodes.end()) return false;
        }
